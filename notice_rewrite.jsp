...
<head>
    ...
    <script src="${contextPath}/views/community/js/notice.js"></script>
...
<body>
    ...
    <div class="middle_space_area">
        ...
        <div class="writing_space">
            <form id="write_frm_btn" action="${contextPath}/free/modWrite.do" method="post" enctype="multipart/form-data">
               <input type="hidden" name="id" value="${user.id}">
               <input type="hidden" name="boardNo" value="${freeVO.boardNo}">
                <div><input type="text" placeholder="제목을 입력해주세요" class="title_go" name="boardName" value="${freeVO.boardName}"></div>
                <div><textarea cols="100" placeholder="내용을 입력해주세요" class="detail_go" name="boardContent">${freeVO.boardContent}</textarea></div>
                <img>${freeVO.imgName}</img>
                <c:if test="${not empty freeVO.imgName}">
				<tr>
					<td width="150" align="center" bgColor="#ff9933" rowspan="2">이미지</td>
					<td><input type="hidden" name="originalFileName" value="${freeVO.imgName}">
					</td>
				</tr>
			</c:if>
                <input type="file" data-ax-path="file" name="boardimg" id="boardimg" multiple="multiple" value="" class="file_file" >
               <input type="submit" value="완료" class="btn1">
                <a class="btn2" onclick="location.href='${contextPath}/free/free_board.do'">목록보기</a>
            </form>
             <%-- <form action="${contextPath}/free/modWrite.do" method="get"  enctype="multipart/form-data">
               <input type="hidden" name="id" value="${log_id}">
            	<input type="hidden" name="boardName" value="${freeVO.boardName}">
            	<input type="hidden" name="boardContent" value="${freeVO.boardContent}">
                <input type="file" data-ax-path="file" name="boardimg" id="boardimg" multiple="multiple" value="" class="file_file" >
            	<input type="hidden" name="boardNo" value="${freeVO.boardNo}">
            	 <input type="submit" value="완료" class="btn1">
              </form> --%>
